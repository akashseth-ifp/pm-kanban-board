...
export const acceptInviteHandler = async (
  ...
  if (email !== loggedInEmail) {
    throw new AppError(`Email mismatch. Login with email ${email}`, 400);
  }
...
export const getBoardMembersHandler = async (
  ...
  if (isAdmin) {
    pendingInvites = await db
      ...
      .from(boardMember)
      .where(
        and(eq(boardMember.boardId, boardId), eq(boardMember.status, "Pending"))
      );
  }
...
export const updateBoardMemberRoleHandler = async (
  ...
  if (!isOwner) {
    if (requesterMembership.role !== "Admin") {
      throw new AppError("Only Admin or Owner can change roles", 403);
    }
